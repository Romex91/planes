...
namespace rplanes
{
	...
	{
		//дань odb. (odb не поддерживает вложенных классов)
...
#pragma db value
		...
		//шаблон самолета. —одержит названи€ всех модулей, установленных на данном самолете профил€
#pragma  db object
		class Plane
		{
		public:
...
#pragma db value
		...
		//профиль игрока. —одержит данные пилота, список названий модулей, содержащихс€ на складе. » конфигурации имеющихс€ самолетов.
#pragma db object
		class Profile
		{
		public:
...
#pragma db id_column("id")\
	value_column("banlist")
			...
			//статистика. обща€ и по самолетам.  ключ - название самолета или total дл€ общей
			...
			//сохранение профил€ включа€ самолеты
			void save(std::shared_ptr< odb::database > profilesDB);
			...
			// ѕокупка и установка модул€ в позицию moduleNo. возвращает строки типа "модуль куплен" и т.п. 
			//если модуль есть на складе,  деньги потрачены не будут. ≈сли moduleNo не корректно, будет возвращено сообщение ошибке.
			...
			//покупка и установка модулей во все подход€щие слоты
			...
			//≈сли модуль есть на складе, цена будет равна 0. ≈сли pos не коректно, будет возвращен пустой массив.
			std::vector<std::pair<std::string, int> > modulePriceList(
				std::string  planeName,
				ModuleType moduleType,
				size_t moduleNo,
				std::shared_ptr<odb::database> planesDB);
			// —писок модулей, содержащихс€ на складе.
			...
			std::string sellPlane(
				std::string planeName,
				std::shared_ptr<odb::database> planesDB);
			// ѕродавать можно только модули, хран€щиес€ на складе. ¬озвращает строки типа "модуль продан"
